...
describe('Card validation form', () => {
  ...
  test('Should be Mastercard', async () => {
    ...
    const form = await page.querySelector('[data-id=validate-card-form]');
    const input = await form.querySelector('[data-id=validate-card-input]');
    await input.type('5536913801114162');
    const submit = await form.querySelector('[data-id=validate-card-submit]');
    submit.click();
